var ClipResult = function (bgProcess) {
	...
	var clippingMsg = operaI18N.getMessage('clipResult_clipping');
	var syncMsg = operaI18N.getMessage('clipResult_sync');
	var successMsg = operaI18N.getMessage('clipResult_success');
	var errorMsg = operaI18N.getMessage('clipResult_error');
	var retryClipMsg = operaI18N.getMessage('retry_clip_button');
	...
	function showNotificationByCmd(cmd, info) {
		console.log('ClipResult.showNotificationByCmd() cmd=' + cmd + 'title=' + info.title);
		switch(cmd) 
		{
		case 'clip' :
			showClipping(info);
	...
	this.showNotificationByCmd = showNotificationByCmd;
}
