...
var com_wizbrother_GlobalUtils = {};
(function(){
  "use strict";
  ...
  com_wizbrother_GlobalUtils.componentizeUrl = function(url) {
    var data = {
      protocol: null,
      domain: null,
      domainNoWww: null,
  ...
  com_wizbrother_GlobalUtils.localize = function(element) {
      ...
      switch (element.type) {
        ...
        case "search":
          if (element.attributes["placeholder"]) {
            var localizedMessage = com_wizbrother_Message.get(element.attributes["placeholder"].value);
            if (localizedMessage) {
              element.placeholder = localizedMessage;
            }
          }
          if (element.attributes["message"]) {
            var localizedMessage = com_wizbrother_Message.get(element.attributes["message"].value);
            if (localizedMessage) {
    ...
    if (element.title){
      var localizedTitle = com_wizbrother_Message.get(element.title);
      if (localizedTitle) {
    ...
    for (var i = 0; i < element.children.length; i++) {
      com_wizbrother_GlobalUtils.localize(element.children[i]);
    }
  }

  com_wizbrother_GlobalUtils.getQueryParams = function(url) {
    var data = com_wizbrother_GlobalUtils.componentizeUrl(url);
  ...
  com_wizbrother_GlobalUtils.escapeXML = function(str) {
    var map = {
      "&" : "&amp;",
      "<" : "&lt;",
      ">" : "&gt;",
  ...
  com_wizbrother_GlobalUtils.createUrlClipContent = function(title, url, favIcoUrl) {
    var titleAttr = (title) ? com_wizbrother_GlobalUtils.escapeXML(title) : "";
  ...
  Object.preventExtensions(com_wizbrother_GlobalUtils);
})();
