...
function LoginControl() {
	...
	function login(loginParam) {
		...
		port.onMessage.addListener(function(msg) {
			if (msg == true) {
				...
				if (!isAutoLogin) {
					//自动登陆不需要再次设置token
					Cookie.setCookies(cookieName, value, expiredays);
				}
			...
			//返回错误
				...
				if (msg == false) {
					msg = com_wizbrother_Message.get('network_wrong');
				}
	...
	function doLogin() {
		var loginingMsg = com_wizbrother_Message.get('logining');
	...
	function checkEmail() {
		...
		if (!valid) {
			$('#userid_error_tip').html(com_wizbrother_Message.get('userid_error')).show(100);
		}
		return valid;
	...
	function checkPassword() {
		...
		if (password.trim().length < 1) {
			$('#password_error_tip').html(com_wizbrother_Message.get('password_error')).show(100);
			return false;
		}
		return true;
	...
	function initCreateAccountLink() {
		$('#create_acount').html(com_wizbrother_Message.get('create_account_link')).bind('click', function(evt) {
